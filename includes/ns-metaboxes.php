...
function ns_control_specifics() {
    ...
    if( 'nanosupport' === $post->post_type ) :
        ...
        $_ns_ticket_form_factor   = get_post_meta( $post->ID, '_ns_ticket_form_factor', true );
        $_ns_ticket_ip_user  = get_post_meta( $post->ID, '_ns_ip_user', true );
        ...
        $meta_internal_reference_establishment = get_post_meta( $post->ID, '_ns_ticket_internal_reference_establishment', true );
        $meta_internal_reference_name = get_post_meta( $post->ID, '_ns_ticket_internal_reference_name', true );
        ...
        $userloginname = get_userdata( $post->post_author );
        ?>
        ...
        <div class="row ns-control-holder">
            ...
            <div class="ns-row misc-pub-section">   
                ...
                <div class="ns-body-col">
                    <div class="ns-field">
                        <a href="<?php echo get_site_url(); ?>/user/<?php echo $userloginname->user_login; ?>" class="ns-field-item" name="ns_internal_rma_author" id="ns-internal-rma-author ns-ticket-status" rows="5" readonly><?php echo  $meta_data_rma_author->first_name . ' ' . $meta_data_rma_author->last_name ?></a>
                    </div> <!-- /.ns-field -->
                </div> <!-- /.ns-box -->
                <div class="ns-body-col">
                    <div class="ns-field">
                    ...
                    <div class="ns-field">
                        ...
                        <?php if ( $status_terms ) {
                            foreach ( $status_terms as $status_term ) {
                                if ( $_ns_ticket_status == $status_term->slug ) {
                                    echo '<option value="'. $status_term->term_id .'" selected="selected">'. $status_term->name .'</option>';
                                } else {
                                    echo '<option value="'. $status_term->term_id .'">'. $status_term->name .'</option>';
                                }
                            }
                        } ?>
            ...
            <div class="ns-row misc-pub-section">
                <div class="ns-head-col">
                    <i class="dashicons dashicons-shield"></i> <?php esc_html_e( 'Form Factor', 'nanosupport' );
                    echo ns_tooltip( 'ns-ticket-status-tooltip', esc_html__( 'Change the device Form Factor.', 'nanosupport' ), 'left' );
                    ?>
                </div>
                <div class="ns-body-col">
                    <div class="ns-field">
                            ...
                            <?php if ( $form_factor_terms ) {
                                foreach ( $form_factor_terms as $form_factor_term ) {
                                    if ( $_ns_ticket_form_factor == $form_factor_term->slug ) {
                                        echo '<option value="'. $form_factor_term->term_id .'" selected="selected">'. $form_factor_term->name .'</option>';
                                    } else {
                                        echo '<option value="'. $form_factor_term->term_id .'">'. $form_factor_term->name .'</option>';
                                    }
                                }
                            } ?>
            ...
            <div class="ns-row misc-pub-section">  
                <div class="ns-head-col">
                    <span class="dashicons dashicons-clipboard"></span> <?php esc_html_e( 'Traking Number', 'nanosupport' );
            ...
            <div class="ns-row misc-pub-section">  
                ...
                <div class="ns-head-col">
                    <span class="dashicons dashicons-clipboard"></span> <?php esc_html_e( 'The establishment name', 'nanosupport' );
                    echo ns_tooltip( 'ns-ticket-status-tooltip', esc_html__( 'The establishment name, where the computer is assigned.', 'nanosupport' ), 'left' );
                    ?>
                </div>
                <div class="ns-body-col">
                    <div class="ns-field">
                        <input type="text" class="ns-field-item" name="ns_ticket_internal_reference_establishment" id="ns_ticket_internal_reference_establishment ns-ticket-status" value="<?php echo isset($_POST['ns_ticket_internal_reference_establishment']) ? $_POST['ns_ticket_internal_reference_establishment'] : esc_html( $meta_internal_reference_establishment ); ?>"></input>
                    </div> <!-- /.ns-field -->
                </div> <!-- /.ns-box -->

                <div class="ns-head-col">
                    <span class="dashicons dashicons-clipboard"></span> <?php esc_html_e( 'The name of assigned technician', 'nanosupport' );
                    echo ns_tooltip( 'ns-ticket-status-tooltip', esc_html__( 'The name of assigned technician of the RMA request.', 'nanosupport' ), 'left' );
                    ?>
                </div>
                <div class="ns-body-col">
                    <div class="ns-field">
                        <input type="text" class="ns-field-item" name="ns_ticket_internal_reference_name" id="ns_ticket_internal_reference_name ns-ticket-status" value="<?php echo isset($_POST['ns_ticket_internal_reference_name']) ? $_POST['ns_ticket_internal_reference_name'] : esc_html( $meta_internal_reference_name ); ?>"></input>
                    </div> <!-- /.ns-field -->
                </div> <!-- /.ns-box -->
...
function ns_save_nanosupport_meta_data( $post_id ) {
    ...
    $ns_ticket_form_factor = $_POST['ns_ticket_form_factor'];
    ...
    wp_set_object_terms($post_id, intval($ns_ticket_status), 'nanosupport_status');

    $get_ns_ticket_status = get_post_meta( $post_id, '_ns_ticket_status', true );
    $term = get_term( intval($ns_ticket_status) , 'nanosupport_status' );

    if ( $term->slug === 'pending' && $get_ns_ticket_status != 'pending' ) {
        remove_action( 'pre_post_update', 'nanosupport_email_on_ticket_update', PHP_INT_MAX, 2);
        remove_action( 'save_post', 'ns_save_nanosupport_meta_data');
        wp_update_post(array(
            'ID'    =>  $post_id,
            'post_status'   =>  'pending'
        ));
        update_post_meta( $post_id, '_ns_ticket_status',   sanitize_text_field( $term->slug ) );
    } elseif ( $term->slug != $get_ns_ticket_status && get_post_status() == 'publish' ) {
        update_post_meta( $post_id, '_ns_ticket_status',   sanitize_text_field( $term->slug ) );
    } elseif ( $term->slug != $get_ns_ticket_status ) {
        remove_action( 'pre_post_update', 'nanosupport_email_on_ticket_update', PHP_INT_MAX, 2);
        remove_action( 'save_post', 'ns_save_nanosupport_meta_data');
        wp_update_post(array(
            'ID'    =>  $post_id,
    ...
    /**
     * Save device form factor.
     * ...
     */
    wp_set_object_terms($post_id, intval($ns_ticket_form_factor), 'nanosupport_form_factor');

    $get_ns_ticket_factor = get_post_meta( $post_id, '_ns_ticket_form_factor', true );
    $form_factor_term = get_term( intval($ns_ticket_form_factor) , 'nanosupport_form_factor' );

    if ( $form_factor_term->slug != $get_ns_ticket_factor ) {
        update_post_meta( $post_id, '_ns_ticket_form_factor', sanitize_text_field( $form_factor_term->slug ) );
    }
    ...
    */
    ...
    /**
    * Save internal reference establishment.
    * ...
    */
    $internal_ticket_internal_reference_establishment = $_POST['ns_ticket_internal_reference_establishment'];
    $existing_ticket_internal_reference_establishment = get_post_meta( $post_id, '_ns_ticket_internal_reference_establishment', true );

    if( $internal_ticket_internal_reference_establishment && $internal_ticket_internal_reference_establishment != $existing_ticket_internal_reference_establishment ) {
        // Sanitize internal note
        $internal_ticket_internal_reference_establishment = wp_kses( $internal_ticket_internal_reference_establishment, ns_allowed_html() );

        update_post_meta( $post_id, '_ns_ticket_internal_reference_establishment', $internal_ticket_internal_reference_establishment );
    } elseif( '' == $internal_ticket_internal_reference_establishment && $existing_ticket_internal_reference_establishment ) {
        delete_post_meta( $post_id, '_ns_ticket_internal_reference_establishment', $existing_ticket_internal_reference_establishment );
    }

    
    /**
    * Save internal reference name.
    * ...
    */
    $internal_ticket_internal_reference_name = $_POST['ns_ticket_internal_reference_name'];
    $existing_ticket_internal_reference_name = get_post_meta( $post_id, '_ns_ticket_internal_reference_name', true );

    if( $internal_ticket_internal_reference_name && $internal_ticket_internal_reference_name != $existing_ticket_internal_reference_name ) {
        // Sanitize internal note
        $internal_ticket_internal_reference_name = wp_kses( $internal_ticket_internal_reference_name, ns_allowed_html() );

        update_post_meta( $post_id, '_ns_ticket_internal_reference_name', $internal_ticket_internal_reference_name );
    } elseif( '' == $internal_ticket_internal_reference_name && $existing_ticket_internal_reference_name ) {
        delete_post_meta( $post_id, '_ns_ticket_internal_reference_name', $existing_ticket_internal_reference_name );
    }
